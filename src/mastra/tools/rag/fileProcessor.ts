...
export const fileProcessorTool = createTool({
  ...
  execute: async ({ context }) => {
    ...
    try {
      ...
      try {
        ...
        for (let i = 0; i < chunks.length; i += batchSize) {
          ...
          const { embeddings } = await embedMany({
            model: models.google.textEmbedding,
        ...
        console.warn(
          `埋め込み処理でエラーが発生しました: ${embeddingError instanceof Error ? embeddingError.message : String(embeddingError)}`,
        );
        return {
          success: true,
          message: `ファイル ${filePath} のチャンキングは成功しましたが、埋め込み処理でエラーが発生しました: ${embeddingError instanceof Error ? embeddingError.message : String(embeddingError)}`,
          filePath,
          strategy,
          chunkCount: chunks.length,
          fileType,
      ...
      return {
        success: false,
        message: `ファイル処理中にエラーが発生しました: ${error instanceof Error ? error.message : String(error)}`,
        filePath,
        strategy,
      };
    }
